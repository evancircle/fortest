...
class BrandDetailTest(unittest.TestCase):
    ...
    def test_search_scanner_id(self):
        url = self.base_url + '/mobile/brand/detail?shop_code=&shop_name=&period_from=2017-07-18&period_to=2017-07-18&scanner_id=BL160080&regionid=0&provinceid=0&cityid=0'
    ...
    def test_search_shop_name(self):
        url = self.base_url + '/mobile/brand/detail?shop_code=&shop_name=中山石岐假日广场店ST&period_from=2017-07-31&period_to=2017-07-31&scanner_id=&regionid=0&provinceid=0&cityid=0'
        r = requests.get(url, cookies=self.cookie)
        self.result = r.json()
        self.assertEqual(self.result['errcode'], 0)
    ...
    def test_search_shop_code(self):
        url = self.base_url + '/mobile/brand/detail?shop_code=DBI3BL&shop_name=&period_from=2017-07-31&period_to=2017-07-31&scanner_id=&regionid=0&provinceid=0&cityid=0'
        r = requests.get(url, cookies=self.cookie)
        self.result = r.json()
        self.assertEqual(self.result['errcode'], 0)
    ...
    def test_search_cityid(self):
        url = self.base_url + '/mobile/brand/detail?shop_code=&shop_name=&period_from=2017-07-31&period_to=2017-07-31&scanner_id=&regionid=100003&provinceid=150000&cityid=150300'
        r = requests.get(url, cookies=self.cookie)
        self.result = r.json()
        self.assertEqual(self.result['errcode'], 0)
